...
import { ShieldAlert, Thermometer, Zap, BatteryCharging, AlertTriangle, CheckCircle2, ChevronRight, Info } from "lucide-react";
import { useBattery } from "@/lib/battery/store";
import { Panel, StatusChip, InfoValue } from "@/components/battery/ui";
...
function FaultDetectionPage() {
  ...
  // Default selected fault is "imbalance"
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Real-time anomaly identification and early-warning diagnostic indicators
        </p>
      </div>
        ...
        {snapshot.faults.map((fault) => {
          ...
          return (
            <button
              ...
              className={`card-surface flex flex-col justify-between p-4 text-left transition-all ${
                isSelected
      ...
      {/* Selected Fault Details Panel */}
      {selectedFault && (
        <Panel
          title={`Fault Detail: ${selectedFault.name}`}
          description="Detailed telemetry diagnostic parameters & historical comparisons"
          ...
          <div className="space-y-5">
            ...
            <div
              className={`rounded-xl p-4 text-xs ${
                selectedFault.status === "warning"
                  ? "bg-warning-soft text-warning-foreground"
                  : selectedFault.status === "critical"
            ...
            >
              <div className="flex items-start gap-2.5">
                ...
                <div>
                  <p className="font-semibold">Diagnostic Recommendation</p>
                  <p className="mt-0.5 leading-relaxed">{selectedFault.note}</p>
                </div>
              </div>
            </div>
          </div>
